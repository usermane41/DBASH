...
// Pour background : output vide, on ne bloque pas sur waitpid
static std::pair<uint32_t, std::string> fork_and_exec(const std::string& message, bool background) {
...
// Helper pour construire et envoyer la réponse
static std::string build_response(uint32_t global_pid, const std::string& output, bool background) {
    if (background)
        return global_pid ? "global_pid:" + std::to_string(global_pid) : "error: fork failed";
    return output.empty() ? "global_pid:" + std::to_string(global_pid) : output;
}
        ...
        switch(type) {
            ...
            case MessageType::ClientCommand: {
...

                {
                    ...
                    auto [global_pid, output] = fork_and_exec(message, background);
