...
// Redirections en attente : target_node_id -> client_sender_id
std::map<int, std::string> pending_redirections;

// Background jobs en attente de notification : local_pid -> {global_pid, client_sender_id}
std::mutex bg_mutex;
...
// Thread de surveillance — attend tous les processus background
// Notifie le receive loop via socket PAIR inproc://jobdone
void watchdog_thread(zmq::context_t& context) {
    ...
    while (true) {
        int status;
        pid_t pid = waitpid(-1, &status, 0);  // attend n'importe quel enfant
        if (pid <= 0) continue;

        // Chercher ce pid dans background_jobs
        bg_mutex.lock();
        ...
        // Notifier le receive loop : "global_pid:XXX|client_id"
...
// Retourne {global_pid, output}
// Foreground : lit l'output via pipe, bloque sur waitpid
// Background : enregistre dans background_jobs pour surveillance, output vide
...
int main(int argc, char* argv[]) {
    ...
    // Socket PAIR pour recevoir les notifications du watchdog
    ...
    while (true) {
        ...
        switch(type) {
            ...
            case MessageType::RemoteCommand: {
                ...
                // sender_id = "0" → répondre via notre dealer vers node 0
