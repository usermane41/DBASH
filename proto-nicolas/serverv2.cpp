...
static std::vector<std::string> split_command(const std::string& cmd_str) {
    ...
    std::string token;
    while (iss >> token) {
        if (token == "&") break;  // on ignore le & ici, géré via background flag
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        switch(type) {
            case MessageType::Heartbeat: {
                int indice = (sender_id.substr(0, 10) == "heartbeat_") ? std::stoi(sender_id.substr(10)) : std::stoi(sender_id);
    ...
    if (my_load > global_charge * 1.5 && target != my_node_id) {
        ...
        if (target != my_node_id) {
            // TODO redirection vers peers[target].dealer
            
            std::cout << "[LB] Redirecting to node " << target << "\n";
        }
    }

    // ---- Fork/exec local (si pas redirigé) ----
    ...
    if (tokens.empty()) {
        response_payload = "error: empty command";
    } else {
        pid_t pid = fork();
        if (pid < 0) {
            response_payload = "error: fork failed";
        } else if (pid == 0) {
            ...
            // ---- Heartbeat réactif après fork ----
            std::string new_load = getLoadAverage();
            for (int j = 0; j < num_nodes; ++j) {
                if (j == my_node_id) continue;
                peers_mutex.lock();
                Liveness status = peers[j].status;
                peers_mutex.unlock();
                if (status != Liveness::Alive) continue;
            ...
            if (background) {
                response_payload = "global_pid:" + std::to_string(global_pid);
            } else {
                int status;
