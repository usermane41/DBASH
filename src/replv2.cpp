...
// Socket ZeroMQ global — connecté au serveur local au démarrage
...
static void execute_command(const Command& cmd)
{
    ...
    // Construire la string de commande à envoyer au serveur
    // Format : "ls -la /tmp" — les args joints par des espaces
    ...
    // Recevoir la réponse
    // Le serveur renvoie : "global_pid:1234" pour background
    //                   ou "output:..." pour foreground (à implémenter côté serveur)
    auto [type, payload] = zmq_recv();
    std::cout << "DEBUG payload: [" << payload << "]\n";  // temporaire
    if (type == MessageType::ClientReturnValue) {
        // Le serveur nous renvoie le global_pid pour les jobs background
        // Format attendu : "global_pid:<valeur>"
        if (payload.substr(0, 11) == "global_pid:") {
            ...
            j.finished   = !cmd.background;  // foreground = déjà terminé
            add_job(j);
        } else {
            std::cout << payload;
        }
...
{
    // ---- Charger la config pour trouver l'endpoint local ----
    ...
    // ---- Connexion ZeroMQ au serveur local ----
