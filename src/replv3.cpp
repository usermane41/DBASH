...
// Appelé après chaque commande et dans la boucle wait
static void check_job_notifications()
{
    zmq::pollitem_t items[] = {
        { static_cast<void*>(zmq_dealer), 0, ZMQ_POLLIN, 0 }
    };
    // Poll non-bloquant (timeout = 0)
    while (zmq::poll(items, 1, std::chrono::milliseconds(10)) > 0) {
        if (items[0].revents & ZMQ_POLLIN) {
            auto [type, payload] = zmq_recv();
            if (type == MessageType::JobFinished) {
                // payload = "global_pid:XXX"
...
static void execute_command(const Command& cmd)
{
    ...
    // Construire la string de commande
    ...
    // Pour le foreground on attend la réponse directement
    // Pour le background aussi — le serveur répond immédiatement avec global_pid
    // Recevoir la réponse — peut recevoir des JobFinished en chemin
        ...
        while (true) {
            ...
            if (t == MessageType::JobFinished) {
                ...
                uint32_t gpid = std::stoul(p.substr(11));
                std::lock_guard<std::mutex> lock(job_mutex);
                for (Job& j : job_table) {
                    if (j.global_pid == gpid) {
                        j.finished = true;
                ...
                continue;  // continuer à attendre la vraie réponse
            }
            type = t;
            payload = p;
            break;
    ...
    if (type == MessageType::ClientReturnValue) {
        if (payload.substr(0, 11) == "global_pid:") {
            // Format foreground : "global_pid:XXX\n<output>"
            // Format background : "global_pid:XXX"
    ...
    // Vérifier les notifications en attente après chaque commande
    check_job_notifications();
}
